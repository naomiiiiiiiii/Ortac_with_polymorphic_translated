...
type tvsymbol = { tv_name : Ident.t } [@@deriving show, sexp]
...
type ty = { ty_node : ty_node } [@@deriving show, sexp_of]

(*have all the ty symbols.
so I have the ident and the polymorphism

but not what the types are applied to, hilariously.

so when you have a list what information do you have remaining
about it by the time you get to drv? <- test this
...
for just list, get
  ...
  so you have recorded the fact that it's polymorphic.ts_ident

  really want a ty_node for every field

  interestingly declared types are also in the namespace under type symbols
*)
(*this representation is quite complicated for converting to
qcheck, probably still best to make your own type, not sure about this*)
and ty_node = Tyvar of tvsymbol | Tyapp of tysymbol * ty list
[@@deriving show, sexp_of]

and tysymbol = {
  ts_ident : Ident.t; (*this is the representation of a type?
                       it's just a string?
                      Int is <Int ident> []*)
  ts_args : tvsymbol list;
  (* we need to keep variables to do things like
     type ('a,'b) t1  type ('a,'b) t2 = ('b,'a) t1 *)
  ts_alias : ty option; (*do get a ty option in here in the
                        declaration case ooo = int list *)
}
[@@deriving show, sexp]
...
module Ts = struct
  ...
  let sexp_of_t = sexp_of_tysymbol
  let t_of_sexp = tysymbol_of_sexp
end
...
type xsymbol = { xs_ident : Ident.t; xs_type : exn_type } [@@deriving show, sexp_of]

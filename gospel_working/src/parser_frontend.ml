...
let parse_ocaml_lb lb =
  let lb_string = Pps.run lb in
  (* Printf.printf "proccessed file:\n%s" lb_string; *)
  let lb_pps = lb_string |> Lexing.from_string in
...
let parse_ocaml_gospel path =
 ...
 let out =  parse_gospel ~filename:path ocaml module_name in
out

...
%{
  ...
  let empty_vspec = {
    ...
    sp_pure = false;
    sp_equiv = [];
    sp_text = ""; (*text of vspec starts out empty*)
...
| PURE bd=val_spec_body
  { {bd with sp_pure = true} } (*adds nothing to text*)
| DIVERGES bd=val_spec_body
  { {bd with sp_diverge = true} } (*adds nothing to text*)
...
| EQUIVALENT e=STRING bd=val_spec_body
  { { bd with sp_equiv = e :: bd.sp_equiv} } (*none of these add anything to sp_text*)

(*sp_text is empty when parsed, must be added somewhere else*)
;
...
term_: (*returns a term description*)
| term_arg_
    { $1 }
| NOT term
    { Tnot $2 }
...
| l = term ; o = infix_op_1 ; r = term 
                                    { (* Printf.printf "%d inside uparser\n%!" (fst $loc).pos_cnum; *) 
      Tinfix (l, o, r) }

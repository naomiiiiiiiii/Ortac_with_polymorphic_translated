...
where payload is PStr of structure | PSig of signature
  | PTyp of core_type | PPat of pattern * expression option
*)
let is_spec attr = attr.attr_name.txt = "gospel"

(*gives the first spec found in a list of attributes,
...
let get_spec_content attr = (*this is what a specification attribute looks like*)
      ...
      [
        {
          pstr_desc =
            Pstr_eval
              ( {
                  pexp_desc = Pexp_constant (Pconst_string (_spec, spec_loc, _third));
                  _;
                  (*need to change the spec text that's put in the attribute
                  when the FIRST signature is made*)
                },
                _ ); (*this second element is a list of attributes*)
          _;
        };
      ] ->
    (*Printf.printf
             "spec_txt in uattr2spec: %s\nend spec_txt\nthird: %s\nend_third\n%!" spec
            ((function | None -> "None" | Some s -> s) third)
      ; *) let _spec_test = "[@@gospel\n
# 4 \"example.mli\"\n
 {| t = silly_create c\n
requires c >= 5\n
requires c < 10\n
...
|}\n
# 8 \"example.mli\"\n]" in
    (*can't use _spec_text for sp_Text or you get a totally incomprehensible error*)

      (_spec, spec_loc) (*this spec is the spec_string
                       which is the original thing... *)
  | _ -> assert false

let get_inner_spec attr =
  match attr.attr_payload with (*the first element is an attribute*)
  | PStr [ { pstr_desc = Pstr_eval (_, attrs); _ } ] -> get_spec_attr attrs
  | _ -> assert false

let parse_gospel ~filename parse attr =
  let spec, spec_loc = get_spec_content attr in (*spec text is the spec here*)
...
(*Ocaml type declaration to a uast type declaration*)
let type_declaration ~filename (t : type_declaration) =
...
let val_description ~filename v =
  ...
  let parse attr =
    let sp_text, spec = parse_gospel ~filename Uparser.val_spec attr in (*sp text is set here*)

...
279
start pos is
365 should be 319..46 too high
end pos is
375 (*10 char length which is actually correct *)
(*it puts the requires first in the tast for nor eason*)
" sill"
  ...
  if
    not
      (try
       ...
       | e ->
           ((Ortac_runtime.Specification_failure
               { term = " sill"; term_kind = Pre; exn = e }) (*this line is better*)
           ...
           if
             not
               (try
                ...
                | e ->
                    ((Ortac_runtime.Specification_failure
                        { term = "< 10\n  rai"; term_kind = Check; exn = e })
                       |> (Ortac_runtime.Errors.register __error__005_);
                     true))
           then
             (Ortac_runtime.Uncaught_checks { term = "< 10\n  rai" }) |>
               (Ortac_runtime.Errors.register __error__005_);
           Ortac_runtime.Errors.report __error__005_);
          raise __e___006_)
